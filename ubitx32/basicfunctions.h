...
{
  File auxfile=FFat.open(namefile,letrar);
  if (auxfile)
    {
...
char* t(int pos)
{
  ...
  File auxfile=FFat.open(auxlang,letrar);
  if (auxfile)
    {
...
char* c(int pos)
{
  File auxfile=FFat.open(filecommon,letrar);
  if (auxfile)
    {
    auxfile.seek(42*(pos-1), SeekSet);
    auxfile.readBytes(auxdesc,42);
...
void savedescr(char* namefile, char* descr, byte ind, byte len)
{
  File auxfile=FFat.open(namefile, wmas);
  if (auxfile)
    {
...
void addlog(byte tipo, int code, char *texto)
{
  File auxfile=FFat.open("/log.txt","a+");
  if (auxfile)  
    { 
...
void addlog(byte tipo, int code, PGM_P texto)
{
  File auxfile=FFat.open("/log.txt","a+");
  if (auxfile)  
    { 
...
void printconceros(int value)  {  if (value<10) s2(0); s2(value); }
...
void saveconf()
{
  ...
  File auxfile=FFat.open(fileconf, wmas);
  if (auxfile) 
    { 
...
{
  int count=0;
  File auxfile=FFat.open(filememo,letrar);
  if (auxfile)
    {
...
void savememo()
{
  File auxfile=FFat.open(filememo, wmas);
  if (auxfile) 
    { 
...
void printlinea(PGM_P texto) { for (byte i=0;i<20;i++) s2(texto); s2(crlf); }
...
void printP(PGM_P texto1) { char c;  while ((c = pgm_read_byte(texto1++))) msg += c; }
void printP(PGM_P texto1, PGM_P texto2) { printP(texto1); printP(texto2);}
void printP(PGM_P texto1, PGM_P texto2, PGM_P texto3) { printP(texto1, texto2); printP(texto3);}
void printP(PGM_P texto1, PGM_P texto2, PGM_P texto3, PGM_P texto4)
  { printP(texto1,texto2);  printP(texto3,texto4); }
void printP(PGM_P texto1, PGM_P texto2, PGM_P texto3, PGM_P texto4, PGM_P texto5)
  { printP(texto1, texto2, texto3, texto4);  printP(texto5); }
void printP(PGM_P texto1, PGM_P texto2, PGM_P texto3, PGM_P texto4, PGM_P texto5, PGM_P texto6)
  { printP(texto1, texto2, texto3, texto4, texto5); printP(texto6); }

void printI(int value)  { printP(itoa(value, buff, 10));  }
void printH(int value)  { printP(itoa(value, buff, 16));  }
void printL(long value)  { printP(ltoa(value, buff, 10));  }

void printF(float value, byte deci) {
...
void printIP(long valor, const  char *texto) { printI(valor); printP(texto); }

void printPiP(const char *texto1, int num, const char *texto2)
...
void printOpc(boolean colorea, boolean activa, char* texto)
{
  if (activa)
...
void printOpc(boolean colorea, boolean activa, PGM_P texto)
{
  if (activa)
    printP(th, texto, th_f);
  else  {
...
void printOpc(boolean colorea, boolean activa, PGM_P texto, PGM_P link)
{
  if (activa)
    printP(th, texto, th_f);
  else  {
...
void printOpc(boolean colorea, boolean activa, PGM_P texto, PGM_P link, byte i)
{
  if (activa)
    printP(th, texto, th_f);
  else  {
...
void printtiempo(unsigned long segundos)
{
  if (segundos < 60)  {
...
#include <TimeLib.h>  // asegúrate de incluirla
...
String generarJSON() {
  String json = "{";
  json += "\"rows\":[";
  json += "[\"Frecuencia\",\"" + String(14.127) + "\",\"USB\"],";
  json += "[\"Modo\",\"" + String("USB") + "\",\"Activo\"],";
  json += "[\"Potencia\",\"" + String(43) + "\",\"Normal\"]";
  json += "],";
  json += "\"state\":\"Estado: OK - Hora: " + getHora() + "\"";
  json += "}";
  return json;
}
